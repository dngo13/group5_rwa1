        ...
        while(shipment_product_count <= kit.products.size()){
          ...
          for(auto &iter: parts_for_kitting){
            ...
            if (cam.faulty_part_list_.size() > 0 ){
              ...
              cam.faulty_part_list_.at(0).world_pose.position.z += 0.08;
              arm.pickPart(parts_to_check_later.at(0).type, cam.faulty_part_list_.at(0).world_pose);
              ROS_INFO_STREAM(cam.faulty_part_list_.at(0).world_pose);
              arm.goToPresetLocation("home2");
                    ...
                    if(comp_class.high_priority_announced && !order1_done){
                      while(true){
                        ...
                        if(temp_order_list.size() > 1){
                          ...
                          /// Order 1 Assembly
                          if (orders.at(1).assembly.size() > 0){
